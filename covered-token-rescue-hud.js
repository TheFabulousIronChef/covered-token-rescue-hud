Hooks.on('updateOverlapHUD', async (token, hover)=>{
  ...
  let topLeft1_mod = { x: c.x + c.w * 0.3,  y: c.y + c.h * 0.3};
  let bottomRight1_mod = { x: c.x + c.w * 0.7, y: c.y + c.h * 0.7 };
  ...
  for (let t of canvas.tokens.placeables.filter(t=>t.visible)) {
    if (t.id==c.id) continue;
      ...
      let topLeft2_mod = { x: t.x + t.w * 0.3 , y: t.y + t.h * 0.3};
      let bottomRight2_mod = { x: t.x + t.w * 0.7 , y: t.y + t.h * 0.7 };
      ...
      if (canvas.grid.type <= 1 && topLeft1.x <= topLeft2.x && topLeft1.y <= topLeft2.y && bottomRight1.x >= bottomRight2.x && bottomRight1.y >= bottomRight2.y && t.owner) 
        covered.push(t);
      if (canvas.grid.type > 1 && doOverlap(topLeft1_mod, bottomRight1_mod, topLeft2_mod, bottomRight2_mod))
        covered.push(t);
  }
